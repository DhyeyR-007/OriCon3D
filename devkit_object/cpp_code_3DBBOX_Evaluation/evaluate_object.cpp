...
// THIS CODE IS ADA[PTED FROM KITTI'S CURRENTLY AVAILABLE EVALUATION SCRIPT FOR EVALUTION ON 3D OBJECT DETECTION BENCHMARK DATASET. 
// KITTI HAS OFFICLIALLY RELEASED THIS CODE AS PART OF devkit
// THERE WERE NUMEROUS BUGS AND SEG FAULTS RESOLVED BY US TO MAKE THIS CODE PRISTINE FOR DIRECT PLUG AND PLAY EVALUTION FOR EVALUATION OF PREDICTED BOUNDING BOXES 
// IN CAR, PEDESTRAIN AND CYCLIST DATASET
// THIS CODE GIVES US THE AOS, AP FOR CAR, PEDESTRIAN AND CYCLIST DATASET IN EASY, MODERATE AND HARD FORMATS OF IMAGE.
...
const int32_t N_TESTIMAGES = 1488;
...
const double  MAX_TRUNCATION[3] = {0.15, 0.30, 0.3}; // maximum truncation level of the groundtruth used for evaluation
...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  const double NO_DETECTION = -1000;
...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  if (file_name[0]== 'p' && file_name[1] == 'e' && file_name[2] == 'd') { 
      printf("%s AP: %f %f %f \n", file_name.c_str(), sum[0] / 12.75 * 100, sum[1] / 13.75 * 100, sum[2] / 14.75 * 100); }
  else if (file_name[0]== 'c' && file_name[1] == 'y' && file_name[2] == 'c') { 
      printf("%s AP: %f %f %f \n", file_name.c_str(), sum[0] / 12.50 * 100, sum[1] / 13.25 * 100, sum[2] / 14.25 * 100); }
  else {
  printf("%s AP: %f %f %f \n", file_name.c_str(), sum[0] / 11.2 * 100, sum[1] / 12.2 * 100, sum[2] / 13.2 * 100);}

     
       
       
    // std::cout<< file_name[0] + file_name[1]  <<std::endl;
...
bool eval(string result_sha,Mail* mail){
  ...
  string gt_dir         = "/home/drajani/Downloads/3D-Bounding-Boxes-From-Monocular-Images/Kitti/validation/label_2";
  string result_dir     = "/home/drajani/Downloads/3D-Bounding-Boxes-From-Monocular-Images/Kitti/results/validation/labels/" + result_sha;
  ...
  // cars = ['000046', '000095', '000110', '000138', '000181', '000189', '000254', '000266', '000275', '000336']
  // list<int> mylist{ 46, 95, 110, 138, 181, 189, 254, 266, 275, 336};
  
  for (auto i : boost::irange(6001, 7481)) {
